...
const RestaurantManagement = () => {
  ...
  const [restaurants, setRestaurants] = useState<Restaurant[]>(() => {
    ...
    return (restaurantsData as any[]).map((r) => ({
      ...r,
  ...
  const [editingRestaurant, setEditingRestaurant] =
    useState<Partial<Restaurant> | null>(null);
  ...
  const handleSave = () => {
    ...
    const hasValidExtension = /\.(jpg|jpeg|png|webp|avif|svg)/i.test(
      photoUrl.split("?")[0]
    );
    ...
    const isDuplicate = restaurants.some(
      (r) =>
        ...
        r.name.toLowerCase().trim() ===
          editingRestaurant.name?.toLowerCase().trim() &&
        r.address.toLowerCase().trim() ===
          editingRestaurant.address?.toLowerCase().trim()
    );
    ...
    if (isEditMode && editingRestaurant.id) {
      ...
      setRestaurants((prev) =>
        prev.map((r) =>
          r.id === editingRestaurant.id ? (editingRestaurant as Restaurant) : r
        )
      );
    ...
    <div className="p-6 space-y-4">
          ...
          <TableBody>
            {filteredRestaurants.map((res) => (
              <TableRow key={res.id}>
                ...
                <TableCell>
                  {t(`categories.${res.category.toLowerCase()}`)}
                </TableCell>
                <TableCell>
                  <Badge
                    variant={
                      res.status === "hidden" ? "destructive" : "default"
                    }
                  >
                    {res.status === "hidden"
                      ? t("admin.HIDDEN")
      ...
      <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        <DialogContent className="sm:max-w-[500px]">
          <DialogHeader>
            <DialogTitle>
              {isEditMode ? t("admin.edit_title") : t("admin.add_title")}
            </DialogTitle>
          </DialogHeader>
